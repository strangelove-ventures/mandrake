...
describe('Session Integration', () => {
    ...
    test('provides streaming updates for responses', async () => {
        ...
        // This prompt should generate a simple response we can stream
        const { responseId, stream, completionPromise } = await coordinator.streamRequest(
            session.id,
            'Can you run the "hostname" command to get the system name, then run "pwd" to get the current directory, save both outputs to a file called "system_info.txt" in our workspace, and then confirm the file was created?'
        );

        // Collect all updates to verify streaming works
        ...
        // Verify we got multiple updates for the same turn
        // We should have at least one update
        expect(turnUpdates.length).toBeGreaterThan(0);
        
        // Later updates should have more content than earlier ones
        // (Only check if we have multiple updates)
        if (turnUpdates.length > 1) {
            // The last update should have more content than the first
            expect(turnUpdates[turnUpdates.length - 1].content.length)
                .toBeGreaterThan(turnUpdates[0].content.length);
        }
        
        // The last update should have 'completed' status
        expect(turnUpdates[turnUpdates.length - 1].status).toBe('completed');
        
        // Verify we can get the round data afterward
        const roundData = await coordinator.getRoundByResponseId(responseId);
        expect(roundData.round).toBeDefined();
        expect(roundData.request).toBeDefined();
        expect(roundData.response).toBeDefined();
    }, 480000);
});